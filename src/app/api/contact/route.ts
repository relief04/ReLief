...
export async function POST(request: Request) {
    try {
        ...
        // 1. SAVE TO DATABASE (Primary Goal)
        ...
        if (dbError) {
            console.error('CONTACT_API_ERROR: Supabase Insert Failed', dbError);
            return NextResponse.json(
                { error: 'Failed to record your message in our database.' },
                { status: 500 }
            );
        }

        // 2. ATTEMPT EMAIL (Optional Fallback / Notification)
        let emailSent = false;
        ...
        if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
            ...
            try {
                ...
                console.warn('CONTACT_API_WARNING: Database insert succeeded, but redundant email alert failed.', emailErr.message);
                // We do not throw an error here because the database successfully saved it!
            }
        }

        return NextResponse.json({
            success: true,
            message: 'Your message has been securely submitted.',
            emailAttempted: process.env.EMAIL_USER ? true : false,
            emailSent: emailSent
        });
    } catch (error: any) {
