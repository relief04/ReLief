...
export async function POST(request: Request) {
    try {
        ...
        for (const event of events) {
            ...
            const attendeeIds = (attendees as any[]).map((a: { user_id: string }) => a.user_id);
            ...
            const emailPromises = (profiles as any[]).map(async (profile: { email: string; username: string | null }) => {
